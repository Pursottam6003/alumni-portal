...
import { EditPencil, InfoCircle as InfoIcon, Trash as TrashIcon, Upload as UploadIcon } from 'iconoir-react';
      ...
      <form className={styles['avatar-upload-form']}>
        <div className={styles['avatar-upload-actions']}>
          <label className={styles.uploadBtn}>
            ...
            <input style={{ display: "none" }} type="file" name="avatar" onChange={handleAvatarChange} />
          </label>
          {(fileUrl) && <Button onClick={removeAvatar}>
            <TrashIcon />
            Remove picture
          </Button>}
          {(avatar !== fileUrl) && (<Button onClick={handleSubmit} className='primary'>
            Save changes
          </Button>)}
        </div>
      </form>
...
const PersonalDetailsForm = ({ prefillData, onSubmit }) => {
  return (
    <SchemaForm prefillData={prefillData} schema={[
      ...
      { name: 'email', label: 'Email (update primary email from account settings)', type: 'email', required: 'Primary email is required', disabled: true },
...
const PersonalDetails = () => {
  ...
  const fetchProfile = () => {
    profileApi().then(data => {
      if (data?.success) setPersonalDetails(prev => ({ ...prev, ...data.personalDetails }));
    })
  }
      ...
      <section className={cx(styles.box, styles['basic-info-wrapper'])}>
        ...
        <div className={styles['basic-info']}>
          ...
          <div className={styles['basic-info-content']}>
            <h2 className={styles['title']}>{dataValueLookup[personalDetails.title]} {personalDetails.firstName} {personalDetails.lastName}</h2>
            <div className={styles['subtitle']}>
              <p>Class of 2020</p>
              <p className={styles['mono']}>
                <span title='Registration no.'>{personalDetails.registrationNo}</span>|
      ...
      <section className={styles.box}>
        ...
        <div className={styles['box-table']}>
          <div className={styles['box-row']}>
            ...
            <p className={cx(styles.col, styles['value'])}>{personalDetails.dob}</p>
          </div>
          <div className={styles['box-row']}>
            <p className={cx(styles.col, styles['label'])}>Sex</p>
            <p className={cx(styles.col, styles['value'])}>{dataValueLookup[personalDetails.sex]}</p>
          </div>
          <div className={styles['box-row']}>
            <p className={cx(styles.col, styles['label'])}>Category</p>
            <p className={cx(styles.col, styles['value'])}>{dataValueLookup[personalDetails.category]}</p>
          </div>
          <div className={styles['box-row']}>
            <p className={cx(styles.col, styles['label'])}>Nationality</p>
            <p className={cx(styles.col, styles['value'])}>{personalDetails.nationality}</p>
          </div>
          <div className={styles['box-row']}>
            <p className={cx(styles.col, styles['label'])}>Religion</p>
            <p className={cx(styles.col, styles['value'])}>{personalDetails.religion}</p>
          </div>
        </div>
      </section>
      ...
      <section className={styles.box}>
        ...
        <div className={styles['box-table']}>
          ...
          <div className={styles['box-row']}>
            <div className={styles['col']}>
              ...
              <p className={styles['value']}>{personalDetails.city}, {personalDetails.state}</p>
              <p className={styles['value']}>{`${personalDetails.country} (${personalDetails.pincode})`}</p>
            </div>
            <div className={styles['col']}>
        ...
        <div className={styles['box-table']}>
          ...
          <div className={styles['box-row']}>
            ...
            <p className={cx(styles.col, styles['value'])}>{personalDetails.linkedin}</p>
          </div>
          <div className={styles['box-row']}>
            <p className={cx(styles.col, styles['label'])}>GitHub</p>
            <p className={cx(styles.col, styles['value'])}>{personalDetails.github}</p>
          </div>
        </div>
      </section>
      <ModalComponent isOpen={isModalOpen} modalTitle='Edit personal details' setIsOpen={(val) => { setIsModalOpen(val) }}>
        <section className={styles.box}>
          <PersonalDetailsForm prefillData={personalDetails} onSubmit={updateProfile} />
        </section>
      </ModalComponent>
    </>) : (<>
      <section className={cx(styles.box, styles.alert, styles.info)}>
        <InfoIcon />
        <h3>Fill in your personal details to complete creating your alumni profile</h3>
      </section>
      <section className={styles.box}>
        <PersonalDetailsForm prefillData={personalDetails} onSubmit={updateProfile} />
      </section>
